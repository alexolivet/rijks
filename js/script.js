                    ...
                    setTimeout(function() { //timeout for image load to canvas - start
                      var c = document.getElementById("myCanvas"); //declare the canvas
                        ...
                        if (img === null) { //if no images are returned then display alert
                         ...
                         ctx.drawImage(img,0,0,450,325);//set image and canvas size
                         $("#loader").hide();
                        $("#save-image").show();//show save image button
                    }, 2200); //timeout for image load to canvas - ends
    ...
    resetCanvasButton.onclick = function() {
        location.reload(true);//location reload does not include the POST data
    };//you can change the default cache reload by setting a forceGet parameter.
    //True is fetching the data from the server anew. Default is false
